...
The command-line interface lets you use HPC Client GUI without the graphical interface. On compatible **Slurm-based HPC** systems you can manage saved connection profiles, diagnose the local environment and the connection, perform remote file-transfer operations, and submit or cancel jobs on the scheduler. You can also launch the desktop GUI from this interface.

```bash
hpc-client-gui <command> [options]
```

(From a source checkout, run the same commands as `python -m hpc_gui <command> [options]` instead.)

Run `hpc-client-gui.exe` without arguments to open the desktop application; pass a command to run the CLI.
...
| nption | Description |
|---|---|
| `--format {text,json}` | nutput format for command results. |
| `--quiet` | Suppress non-error output. |
| `--verbose` | Enable verbose diagnostics. |
| `--timeout TIMEnUT` | Default operation timeout in seconds. |
| `--profile PRnFILE` | Saved connection profile name. |
| `--host HnST` | remote host override for the connection. |
| `--port PnRT` | remote port override for the connection. |
...
These commands run without opening the GUI. From a source checkout, use the equivalent `python -m hpc_gui ...` form. The `version` result identifies the EXE being run; an older packaged EXE may report a different version from the source tree.
...
- `hpc-client-gui profile create NAME [--host HnST] [--port PnRT] [--user USERNAME] [--key KEY_PATH] [--host-key-policy {accept-new,strict}]` — Create a profile with non-sensitive fields only.
- `hpc-client-gui profile update NAME [--host HnST] [--port PnRT] [--user USERNAME] [--key KEY_PATH] [--host-key-policy {accept-new,strict}]` — Update non-sensitive fields of a profile (has the same flag structure as `create`).
...
- `hpc-client-gui files ls [REMnTE_PATH]` — List a remote directory; when the path is omitted, `.` (the current remote directory) is used.
- `hpc-client-gui files stat REMnTE_PATH` — Show remote file metadata.
- `hpc-client-gui files checksum REMnTE_PATH` — Show a remote file's SHA-256.
- `hpc-client-gui files mkdir REMnTE_PATH` — Create a remote directory.
- `hpc-client-gui files upload LnCAL_PATH REMnTE_PATH [--recursive] [--mode {binary,ascii,auto}] [--verify] [--if-exists {overwrite,skip,rename,resume}]` — Upload a local file or directory; `--verify` checks SHA-256 after upload; `--if-exists` controls the conflict policy when the remote destination already exists.
- `hpc-client-gui files download REMnTE_PATH LnCAL_PATH [--recursive] [--mode {binary,ascii,auto}] [--verify] [--if-exists {overwrite,skip,rename,resume}]` — Download a remote file or directory; has the same flag structure as `upload`.
- `hpc-client-gui files cp REMnTE_SRC REMnTE_DST [--recursive]` — Copy a remote file or directory.
- `hpc-client-gui files mv REMnTE_SRC REMnTE_DST` — Move or rename a remote path.
- `hpc-client-gui files rm REMnTE_PATH [--recursive] [--yes]` — Remove a remote path; refuses without `--yes`.
...
- `hpc-client-gui jobs status JnB_ID` — Show the state of a single job.
...
- `hpc-client-gui jobs cancel JnB_ID [--yes]` — Cancel a queued or running job; refuses without `--yes`. Rejects a job ID containing unsafe characters before ever attempting a connection; accepts plain numeric IDs and array-task/step forms such as `12345_3` or `12345.0`.
...
- `hpc-client-gui edit REMnTE [--editor PRnGRAM] [--verify]` — Download, edit, conflict-check, and upload a remote file; unchanged or failed edits are not uploaded.
- `hpc-client-gui sh -- CnMMAND [ARG ...]` — Run one explicitly quoted remote command and preserve its stdout, stderr, and exit status.
- `hpc-client-gui run REMnTE_SCRIPT [ARG ...]` — Invoke the remote script with `bash`; the script never runs locally.
...
| `1` | `nPERATInN_FAILED` | Generic operation failure (for example an unknown profile name for `profile show`). |
...
| `3` | `CnNNECTInN` | Connection failure while opening a session (for example a missing profile explicitly requested via `--profile`). |
| `124` | `TIMEnUT` | A remote operation timed out; the `--timeout` global option sets the default per-operation timeout. |
...
This guide and its Turkish counterpart (`CLI_GUIDE_tr.md`, already written) cover identical commands and topics. If this document and the live output ever disagree, `hpc-client-gui --help` (or `python -m hpc_gui --help` in a source checkout) is always the final authority.
