...
except ImportError:  # pragma: no cover - exercised by unsupported dev installs
    QWebChannel = QWebEnginePage = QWebEngineView = None


if QWebEnginePage is not None:
    class _LocalTerminalPage(QWebEnginePage):
...
class TerminalWidget(QWidget):
    ...
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        if QWebEngineView is None or QWebChannel is None or QWebEnginePage is None:
            raise RuntimeError("Qt WebEngine is unavailable in this installation")
        self.bridge = TerminalBridge(self)
        self.view = QWebEngineView(self)
        self.view.setPage(_LocalTerminalPage(self.view))
