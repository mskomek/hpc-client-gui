...
HPC_SYSTEM_DEFAULTS: dict[str, str] = {
    "name": "HPC",
...
def hpc_default_remote_paths() -> dict[str, str]:
    return {
        "scratch_dir": HPC_SYSTEM_DEFAULTS["scratch_dir"],
        "home_dir": HPC_SYSTEM_DEFAULTS["home_dir"],
    }


def builtin_system_template_groups() -> dict[str, list[dict[str, str]]]:
    return {
        "HPC": [dict(HPC_SYSTEM_DEFAULTS)],
    }


def normalize_system_settings(value: Any) -> dict[str, str]:
    settings = dict(HPC_SYSTEM_DEFAULTS)
