...
FEATURE_EVIDENCE = {
    ...
    "GUI-TERM-002": Evidence("COVERED", "main_window.py", "wx_shell.py; wx_local_files.py; wx_remote_files_view.py; wx_editor_view.py", "test_wx_term002.py; test_wx_terminal.py", "Real wx production shell/file/editor paths prove shell-script files are dispatched to the terminal with shell-safe quoting and the editor Run button reaches the same terminal path. Measured on Windows; the complete wx suite retains one unrelated clipboard-environment failure."),
