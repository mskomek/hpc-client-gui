    ...
    def load_logs():
        def worker():
            try:
                ...
                if state["closed"]:
                    return
                wx.CallAfter(lambda: log_view.SetValue(t("logs.read_failed").format(err=str(exc))) if not state["closed"] else None)
    ...
    def export_diagnostics(_event=None):
        ...
        def worker():
            try:
                ...
                if state["closed"]:
                    return
                wx.CallAfter(lambda: wx.MessageBox(t("logs.bundle_failed").format(err=str(exc)), t("logs.diagnostics_title"), wx.OK | wx.ICON_ERROR, host) if not state["closed"] else None)
