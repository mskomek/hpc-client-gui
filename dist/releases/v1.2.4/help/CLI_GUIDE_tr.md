...
```bash
hpc-client-gui <komut> [seçenekler]
```

(Kaynak kod dizininden çalıştırıyorsanız aynı komutları `python -m truba_gui <komut> [seçenekler]` biçiminde çağırın.)

Paketlenmiş konsol çalıştırılabilir dosyası `hpc-client-cli.exe` ile açılır. Komut vermeden çalıştırıldığında `truba>` istemini açar; masaüstü uygulaması için `hpc-client-gui.exe` çalıştırılır.
...
| `--strict-host-key` | Bilinmeyen uzak sunucu anahtarlarını reddeder; değişmiş anahtar her zaman reddedilir. |

Varsayılan `accept-new` CLI politikası ilk görülen anahtarı `~/.truba_slurm_gui/known_hosts` dosyasına kaydeder ve sonraki bağlantılarda doğrular. GUI; güvenip kaydetme, bir kez güvenme veya iptal seçeneklerini sunar. Kayıtlı anahtar değişirse iki arayüz de bağlantıyı iptal eder; bu dosyadaki ilgili sunucu satırını silmeden önce yeni parmak izini doğrulayın.
...
`commands` alt komutu, komut dosyası oluşturma ve otomasyon için komut envanterini yazdırır: tüm komut yolları, seçenekler ve çıkış kodu tablosu. Ayrıntılı bir alt komut yardımı için ilgili komutun sonuna `--help` ekleyin. `--format text|json` biçimini kabul eder ve her zaman kullanılabilir. Genel bayraklar alt komuttan önce yazılmalıdır:
...
Windows'ta paketlenmiş EXE ile yardım doğrulaması:

```cmd
set "EXE=D:\Projeler\truba-client-gui_windows_onedir\hpc-client-cli.exe"
"%EXE%" --help
...
Bu komutlar GUI açmadan çalışır. Kaynak ağaçta eşdeğer çağrı `python -m truba_gui ...` biçimindedir. `version` çıktısı, çalıştırılan EXE'nin sürümünü gösterir; kaynak sürüm ile eski bir EXE'nin sürümü farklı olabilir.
...
hpc-client-gui files rm /remote/path/eski_dizin --recursive --yes
```
...
### Kısa yollar, FTP ve uzak kabuk

Kök kısa yolları kanonik işleyicilere yönlenir: `put`/`get` dosya yükleme/indirmeye; `ls`, `stat`, `checksum`, `mkdir`, `cp`, `mv` ve `rm` eşleşen dosya komutlarına; `squeue`, `scontrol`, `sacct`, `lssrv`, `sbatch` ve `scancel` eşleşen iş komutlarına gider. Aynı eşlemeler `hpc-client-gui --format json commands` çıktısının `aliases` alanında listelenir.

Dosya komutları varsayılan olarak SFTP kullanır. Yalnızca dosya işlemleri için açıkça `--transport ftp` kullanın; zamanlayıcı ve kabuk komutları SFTP/SSH gerektirir. FTP meta verileri ve SHA-256 değeri dosya backend'i üzerinden hesaplanır, uzak kabuk çalıştırılmaz.

Uzak düzenleme ve kabuk komutları:

- `hpc-client-gui edit REMOTE [--editor PROGRAM] [--verify]` — Uzak dosyayı indirir, düzenler, çakışmayı kontrol eder ve yükler; değişmeyen veya başarısız düzenleme yüklenmez.
- `hpc-client-gui sh -- COMMAND [ARG ...]` — Açıkça quote edilmiş tek bir uzak komut çalıştırır; stdout, stderr ve çıkış kodunu korur.
- `hpc-client-gui run REMOTE_SCRIPT [ARG ...]` — Uzak betiği `bash` ile çalıştırır; betik yerelde çalıştırılmaz.
- `hpc-client-gui terminal` — Mevcut SSH terminaline bağlanır; etkileşimli konsol gerektirir.
- `hpc-client-gui interactive` — Girilen komutları aynı CLI kayıt defteriyle ayrıştıran küçük bir metin istemi açar; `exit` ve `quit` çıkar.

Uzak kabuk komutları NUL/kontrol karakterlerini reddeder. `--password-stdin` otomasyon içindir; `--password-prompt` maskelidir ve gerçek bir terminal gerektirir.
