...
For the packaged console executable, run `hpc-client-cli.exe`. With no command it opens the `truba>` interactive prompt; run `hpc-client-gui.exe` to open the desktop application.
