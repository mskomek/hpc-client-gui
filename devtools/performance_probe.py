...
class PerformanceSession:
    def __init__(self, repo_root: Path, *, interval_ms: int = 100, slow_ms: int = 250, now=time.perf_counter) -> None:
        self.repo_root = repo_root.resolve()
        self.source_root = (self.repo_root / "src" / "hpc_gui").resolve()
        self.interval_ms, self.slow_ms, self._now = interval_ms, slow_ms, now
    ...
    def start(self) -> None:
        ...
        self.write_event("session_start", python=sys.version.split()[0], pid=os.getpid(), interval_ms=self.interval_ms, slow_ms=self.slow_ms)

    def mark(self, name: str) -> None:
        self.write_event("startup_checkpoint", name=name, elapsed_ms=round((self._now() - self._started_at) * 1000, 3))
    ...
    def measure_tick(self, observed_at: float) -> float:
        ...
        if delay_ms >= self.slow_ms:
            self.write_event("event_loop_delay", delay_ms=round(delay_ms, 3), threshold_ms=self.slow_ms)
        return delay_ms
    ...
    def finish(self, exit_code: int | None = None) -> None:
        ...
        self.write_event("session_end", exit_code=exit_code, elapsed_ms=round((self._now() - self._started_at) * 1000, 3))
    ...
    def _write_profile_summary(self) -> None:
        ...
        for (filename, line, function), values in stats.stats.items():
            ...
            rows.append({"file": str(Path(filename).resolve().relative_to(self.repo_root)), "line": line, "function": function, "calls": total_calls, "primitive_calls": primitive_calls, "total_ms": round(total_time * 1000, 3), "cumulative_ms": round(cumulative_time * 1000, 3)})
...
def start(repo_root: Path) -> None:
    ...
    if _SESSION is not None:
        return
    _SESSION = PerformanceSession(repo_root, interval_ms=max(10, int(os.environ.get("TRUBA_GUI_PERF_INTERVAL_MS", "100"))), slow_ms=max(1, int(os.environ.get("TRUBA_GUI_PERF_SLOW_MS", "250"))))
    _SESSION.start()
