...
@pytest.mark.audit
def test_trusted_tool_execution_is_confined_to_approved_adapter():
    plugin_dir = Path("src/hpc_gui/plugins")
    loaders = {
        path.name
        for path in plugin_dir.glob("*.py")
        if "exec_module(" in path.read_text(encoding="utf-8")
    }
    assert loaders == {"linter_tools.py"}

    source = (plugin_dir / "linter_tools.py").read_text(encoding="utf-8")
    assert source.index("is_approved_trusted_tool(installed.manifest)") < source.index(
        "spec.loader.exec_module(module)"
    )
    assert 'rel != "engine/ansys_lint/__init__.py"' in source
