...
class FtpWidgetTests(unittest.TestCase):
    ...
    def test_transfer_item_trailing_cached_size_keeps_positional_construction(self) -> None:
        ...
        self.assertFalse(plain.recursive)
        self.assertEqual(plain.priority, "Normal")

        sized = TransferItem("upload", "local.bin", "/remote/local.bin", False, "High", 2048)
        self.assertEqual(sized.cached_size, 2048)
        self.assertEqual(sized.priority, "High")
    ...
    def test_transfer_item_size_prefers_cached_size_without_repeat_stat(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch(
                "pathlib.Path.stat",
                side_effect=AssertionError("cached size must avoid a stat"),
            ):
                self.assertEqual(
                    self.widget.transfer_activity._item_size(item),
                    128,
                )
    ...
    def test_download_plan_carries_listed_remote_size(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            result = panel._build_remote_download_plan_background(
        ...
        transfers = [op for op in plan if op.op == "download"]
        sizes = {op.src: op.size for op in transfers}
        self.assertEqual(sizes.get("/remote/root.txt"), 4)
    ...
    def test_transfer_items_from_plan_preserve_known_size_and_leave_unknown_none(self) -> None:
        plan = [
            ...
            _PlannedOp("upload", "local.txt", "/remote/local.txt"),
            _PlannedOp("mkdir_local", "", r"D:\target\folder"),
        ]
        items = RemoteDirPanel._transfer_items_from_plan(plan)
        self.assertEqual(len(items), 3)
        self.assertEqual(items[0].cached_size, 4096)
    ...
    def test_progress_updates_use_direct_item_id_row_lookup(self) -> None:
        ...
        try:
            ...
            dialog._pending = []
            panel = self.widget.transfer_activity
            panel.record("controller", [dialog], "Download")

            self.assertIn(id(item), panel._row_by_item_id)
            self.assertIn(id(item), panel._progress_bar_by_item_id)
            registered_bar = panel._progress_bar_by_item_id[id(item)]
            self.assertIsNotNone(registered_bar)

            dialog.transferProgressChanged.emit(item, 30, 100)

            row = panel._row_by_item_id[id(item)]
            bar = panel._progress_bar_by_item_id[id(item)]
            self.assertEqual(bar.value(), 30)
            self.assertEqual(
                panel.queue_list.itemWidget(row, panel._PROGRESS_COLUMN),
                bar,
            )

            dialog.transferListsChanged.emit([], [], [])
            self.assertIn(id(item), panel._row_by_item_id)
            self.assertEqual(panel._progress_bar_by_item_id[id(item)].value(), 30)
        finally:
            dialog.deleteLater()
    ...
    def test_transfer_activity_reuses_unaffected_rows_on_status_transition(self) -> None:
        ...
        try:
            ...
            panel.attach_controller(dialog)
            before = [
                panel.queue_list.topLevelItem(index)
                for index in range(panel.queue_list.topLevelItemCount())
            ]
            self.assertEqual(
                [row.text(6) for row in before],
                ["Queued", "Queued", "Queued"],
            )

            dialog._active_items = [items[0]]
            dialog._active_item = items[0]
            dialog._pending = items[1:]
            dialog.transferListsChanged.emit([], [], [])

            self.assertEqual(panel.queue_list.topLevelItemCount(), 3)
            self.assertEqual(
                [panel.queue_list.topLevelItem(index).text(6) for index in range(3)],
                ["Transferring", "Queued", "Queued"],
            )
            self.assertIs(
                panel.queue_list.topLevelItem(0),
                before[0],
            )
            self.assertIs(
                panel.queue_list.topLevelItem(1),
                before[1],
            )
            self.assertIs(
                panel.queue_list.topLevelItem(2),
                before[2],
            )
        finally:
            dialog.deleteLater()
    ...
    def test_transfer_activity_transition_keeps_bounded_cap_and_remaining(self) -> None:
        ...
        try:
            panel = self.widget.transfer_activity
            with patch.object(panel, "_MAX_VISIBLE_TRANSFER_ROWS", 3):
                panel.attach_controller(dialog)
                self.assertEqual(panel.queue_list.topLevelItemCount(), 4)
                self.assertEqual(
                    panel.queue_list.topLevelItem(3).text(2),
                    "Remaining: 2",
                )
                visible_before = [
                    panel.queue_list.topLevelItem(index) for index in range(3)
                ]
            ...
            self.assertEqual(panel.queue_list.topLevelItem(3).text(2), "Remaining: 2")
            self.assertIs(panel.queue_list.topLevelItem(0), visible_before[0])
            self.assertIs(panel.queue_list.topLevelItem(1), visible_before[1])
            self.assertIs(panel.queue_list.topLevelItem(2), visible_before[2])
        finally:
            dialog.deleteLater()
    ...
    def test_transfer_dialog_coalesces_burst_progress_and_delivers_final(self) -> None:
        ...
        try:
            ...
            with patch(
                ...
                dialog._on_transfer_progress(item, 200, 1000)
                dialog._on_transfer_progress(item, 300, 1000)
                dialog._on_transfer_progress(item, 400, 1000)
                self.assertEqual(published, [(100, 1000)])
                dialog._on_transfer_progress(item, 500, 1000)
                dialog._on_transfer_progress(item, 1000, 1000)
            self.assertEqual(published, [(100, 1000), (500, 1000), (1000, 1000)])
            self.assertEqual(dialog._last_progress_by_item[id(item)], (1000, 10.252))
        finally:
            dialog.deleteLater()
    ...
    def test_ssh_backend_parallel_uploads_use_isolated_closed_channels(self) -> None:
        ...
        class FakeSSH:
            sftp = SimpleNamespace(rename=lambda _old, _new: None)
    ...
    def test_upload_preflight_bounds_plan_rows_without_truncating_items(self) -> None:
        items = [
            TransferItem(
                "upload",
                f"C:/local/file-{index}.bin",
                f"/remote/folder/file-{index}.bin",
            )
        ...
        try:
            self.assertEqual(len(dialog._items), 250)
            self.assertEqual(dialog.plan_list.topLevelItemCount(), 201)
            remaining_row = dialog.plan_list.topLevelItem(200)
            self.assertEqual(remaining_row.text(0), "Remaining: 50")
            self.assertEqual(dialog.plan_list.topLevelItem(0).text(1), "C:/local/file-0.bin")
            self.assertIn("250 files", dialog.lbl_summary.text())
            self.assertIn("250 total steps", dialog.lbl_summary.text())
            self.assertEqual(dialog.btn_start.text(), "Start transfer")
        finally:
            dialog.deleteLater()
    ...
    def test_both_remote_panels_forward_batch_submit_signals(self) -> None:
        batches = []
        shell_batches = []
        self.widget.batchSubmitRequested.connect(batches.append)
        self.widget.batchShellRequested.connect(shell_batches.append)

        self.widget.panel_scratch.batch_submit_requested.emit(
            ["/scratch/a.slurm", "/scratch/B.slurm"]
        )
        self.widget.panel_home.batch_submit_requested.emit(["/home/c.sbatch"])
        self.widget.panel_scratch.batch_shell_requested.emit(
            ["/scratch/Z.sh", "/scratch/a.sh"]
        )
        ...
        self.assertEqual(
            RemoteDirPanel._batch_submit_candidates(
                [
                    ...
                    ("/arf/scratch/alice/b.sbatch", False),
                ]
            ),
            ["/arf/scratch/alice/a.slurm", "/arf/scratch/alice/b.sbatch"],
        )
        self.assertEqual(
            RemoteDirPanel._batch_submit_candidates(
                [
            ...
            ["/arf/scratch/alice/a.slurm"],
        )
        self.assertEqual(
            RemoteDirPanel._batch_submit_candidates(
                [
                    ("/arf/scratch/alice/run.sh", False),
                    ("/arf/scratch/alice/notes.txt", False),
                ]
            ),
            [],
        )
        self.assertEqual(
            RemoteDirPanel._batch_shell_candidates(
                [
                    ("/remote/Z.sh", False),
                    ("/remote/a.sh", False),
                    ("/remote/folder.sh", True),
                ]
            ),
            ["/remote/a.sh", "/remote/Z.sh"],
        )
        mixed = [
            ("/remote/job.slurm", False),
            ("/remote/run.sh", False),
        ]
        self.assertEqual(
            RemoteDirPanel._batch_submit_candidates(mixed),
            ["/remote/job.slurm"],
        )
        self.assertEqual(
            RemoteDirPanel._batch_shell_candidates(mixed),
            ["/remote/run.sh"],
        )
    ...
    def test_main_window_routes_ftp_actions_to_existing_directories_handlers(self) -> None:
        ...
        def record_shell_batch(_self, paths):
            shell_batches.append(paths)
        ...
        with (
            ...
            patch.object(MainWindow, "run_shell_batch_in_terminal", record_shell_batch),
        ):
            window = MainWindow()
            try:
                ...
                window.ftp.panel_scratch.batch_shell_requested.emit(
                    ["/scratch/Z.sh", "/scratch/a.sh"]
                )
    ...
    def test_main_window_batch_shell_uses_login_terminal_order(self) -> None:
        ...
        try:
            with (
                patch.object(window.tabs, "setCurrentWidget") as select_tab,
                patch.object(window.login, "run_command_text") as run_command,
            ):
                window.run_shell_batch_in_terminal(
                    ["/remote/Z name.sh", "/remote/a.sh"]
                )
            select_tab.assert_called_once_with(window.login)
            run_command.assert_called_once_with(
                "bash '/remote/Z name.sh'\n" "bash /remote/a.sh"
            )
        finally:
            window.graceful_shutdown()
            window.deleteLater()
    ...
    def test_batch_submit_worker_orders_sequentially_and_continues_after_failure(self) -> None:
        class RecordingSlurm:
            def __init__(self) -> None:
                self.calls: list[str] = []
                self.fail_on = {"/remote/broken.slurm"}

            def sbatch(self, script_path: str) -> str:
                self.calls.append(script_path)
                if script_path in self.fail_on:
                    raise RuntimeError("simulated failure")
                return f"Submitted batch job {1000 + self.calls.index(script_path)}"

        slurm = RecordingSlurm()
        worker = _BatchSubmitWorker(
            slurm,
            ["/remote/a.slurm", "/remote/broken.slurm", "/remote/c.slurm"],
        )
        finished_files: list[tuple[str, str]] = []
        failed_files: list[tuple[str, str]] = []
        done: list = []
        worker.signals.file_finished.connect(
            lambda _worker, path, output: finished_files.append((path, output))
        )
        worker.signals.file_failed.connect(
            lambda _worker, path, error: failed_files.append((path, error))
        )
        ...
        self.assertEqual(
            [path for path, _output in finished_files],
            ["/remote/a.slurm", "/remote/c.slurm"],
        )
        self.assertEqual(
            [path for path, _error in failed_files],
            ["/remote/broken.slurm"],
        )
    ...
    def test_main_window_contains_top_level_ftp_tab(self) -> None:
        ...
        try:
            ...
            self.assertIs(window.ftp, window.tabs.widget(labels.index("Files")))
        finally:
            window.graceful_shutdown()
            window.deleteLater()
    ...
    def test_download_conflict_planning_keeps_remote_probes_off_gui_thread(self) -> None:
        class ProbeGuardingFiles:
            ...
            def __init__(self, gui_thread: int) -> None:
            ...
            def _probe(self) -> int:
                ...
                if thread_id == self.gui_thread:
                    raise AssertionError(
                        "remote download planning must not probe the remote backend "
                        "on the GUI thread, even for a conflict"
                    )
                self.probe_threads.append(thread_id)
                return thread_id
            ...
            def listdir_entries(self, path: str):
                self._probe()
                return list(self.entries.get((path or "/").rstrip("/") or "/", []))
        ...
        gui_thread = threading.get_ident()
        files = ProbeGuardingFiles(gui_thread)
        panel = self.widget.panel_scratch
        panel.session = {"connected": True, "files": files}
        panel._show_transfer_dialog = False
        RemoteDirPanel._session_conflict_action = None
        try:
            with tempfile.TemporaryDirectory() as tmp:
                conflict_target = Path(tmp, "remote", "root.txt")
                conflict_target.parent.mkdir()
                conflict_target.write_text("local", encoding="utf-8")

                with (
                    patch.object(
                        TransferConflictDialog,
                        "get_decision",
                        return_value=TransferConflictDecision("skip"),
                    ) as decision,
                    patch.object(panel, "_run_plan_with_progress") as run_plan,
                ):
                    started_at = time.monotonic()
                    self.assertTrue(
                        panel._apply_remote_download_incremental(["/remote"], tmp)
                    )
                    self.assertLess(time.monotonic() - started_at, 0.2)
            ...
            self.assertTrue(files.probe_threads)
            self.assertTrue(
                all(thread_id != gui_thread for thread_id in files.probe_threads)
            )
            plan = run_plan.call_args.args[0]
            self.assertNotIn(
                str(conflict_target),
                [op.dst for op in plan if op.op == "download"],
            )
            self.assertEqual(
                [(op.op, op.src) for op in plan if op.op == "download"],
                [("download", "/remote/child/nested.txt")],
            )
        finally:
            RemoteDirPanel._session_conflict_action = None
    ...
    def test_download_with_mode_resumes_from_part_and_replaces_final(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(files.calls, [str(part)])
    ...
    def test_local_folder_middle_click_opens_new_directory_tab(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(local.set_dir(tmp))
            item = next(
                local.tree.topLevelItem(index)
                for index in range(local.tree.topLevelItemCount())
                if local.tree.topLevelItem(index).text(0) == "project"
            )
            event = FakeMiddleRelease()
            ...
            with patch.object(
                ...
                local.tree.mouseReleaseEvent(event)
    ...
    def test_local_middle_click_ignores_file_parent_and_blank_space(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            (Path(tmp, "file.txt")).write_text("data", encoding="utf-8")
            local = self.widget.local_panel
            self.assertTrue(local.set_dir(tmp))
            parent_item = next(
                local.tree.topLevelItem(index)
                for index in range(local.tree.topLevelItemCount())
                if local.tree.topLevelItem(index).text(0) == ".."
            )
            file_item = next(
                local.tree.topLevelItem(index)
                for index in range(local.tree.topLevelItemCount())
                if not bool(
                    local.tree.topLevelItem(index).data(
                        0,
                        Qt.ItemDataRole.UserRole + 1,
                    )
                )
            ...
            with patch.object(
                ...
                for clicked_item in (file_item, parent_item, None):
                    event = FakeMiddleRelease()
                    with patch.object(local.tree, "itemAt", return_value=clicked_item):
                        local.tree.mouseReleaseEvent(event)
                    self.assertTrue(event.accepted)
