...
class MockClusterRoundTripTests(unittest.TestCase):
    ...
    def _run_cli(self, *args: str) -> subprocess.CompletedProcess:
        ...
        env = {
            **os.environ,
            **env,
            "HOME": self._temp.name,
            "USERPROFILE": self._temp.name,
        }
        cmd = [
            sys.executable,
            "-m",
    ...
    def test_files_round_trip_over_real_ssh_wire(self) -> None:
        ...
        known_hosts = self.root_dir / ".truba_slurm_gui" / "known_hosts"
        self.assertTrue(known_hosts.is_file())
        saved_host_key = known_hosts.read_text(encoding="utf-8")
        ...
        self.assertEqual(known_hosts.read_text(encoding="utf-8"), saved_host_key)
