...
$results.shared_home_job = Invoke-LabSshCapture $controller.ip 'cd /srv/hpc/home/hpctest && rm -rf .local-real-job && mkdir .local-real-job && printf "#!/bin/sh\nprintf \"%%s %%s\\n\" \"\$(hostname)\" \"\$PWD\" >> /srv/hpc/home/hpctest/.local-real-job/result\n" > .local-real-job/job.sbatch && sbatch --wait --parsable -N2 -n2 .local-real-job/job.sbatch >/tmp/local-real-job.id && test "$(wc -l < .local-real-job/result)" -eq 2 && grep -q "^compute01 /srv/hpc/home/hpctest/.local-real-job$" .local-real-job/result && grep -q "^compute02 /srv/hpc/home/hpctest/.local-real-job$" .local-real-job/result'
