...
foreach ($required in @('windows-latest', 'python-version: "3.14"', 'Windows boundary tests', 'tests/test_safe_download.py')) { if ($ci -notmatch [regex]::Escape($required)) { throw "Missing Windows CI gate: $required" } }
