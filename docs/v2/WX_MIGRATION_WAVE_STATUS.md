...
| 74 Input/Keyboard/Paste/Unicode/PTY | xterm onData → bridge → `send_shell_input`, full Ctrl A-_, nav keys, multiline paste, Unicode, FitAddon PTY re-fit | `wx_terminal_webview.py` _handle_input/hpc_write/hpc_paste + `wx_bridge.js` terminal.onData | **VERIFIED_COMPLETE** | `test_wx_terminal_input_chain_ctrl_a_to_z`, `test_wx_terminal_unicode_input_output`, `test_wx_terminal_multiline_paste`, `test_wx_terminal_resize_chain_to_ssh`, `test_wx_terminal_font_change_triggers_resize` | Real input chain, Ctrl+A-Z, Unicode, multiline paste, FitAddon resize | **VERIFIED_COMPLETE** | — |
