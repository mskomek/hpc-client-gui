...
| 76 Behavioral Evidence Gate | real wx→WebView→production bridge/adapter with fake SSH callbacks, visible xterm fixtures, stress invariants | `test_wx_terminal_parity_evidence.py` (10 GUI behavior tests) + historical evidence JSON | **PARTIAL** | `test_vt_sgr_*`, `test_vt_carriage_return_*`, `test_unicode_round_trip`, `test_multiline_paste`, `test_resize_*`, `test_stress_*`, `test_close_while_output_in_flight` | The fake adapter tests do not launch a PTY or prove a real terminal process. Alternate-screen and current packaged keyboard-to-output evidence remain incomplete. The former test-local JSON writer was removed because it overwrote real evidence with hard-coded claims. | **PARTIAL** | Prove complete visible behavior chain |
