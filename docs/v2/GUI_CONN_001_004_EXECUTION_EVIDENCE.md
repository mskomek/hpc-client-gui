...
`python scripts/check_i18n.py` – still FAILED only on pre-existing unrelated missing references (125, e.g. `splash.*`, `updates.*`); no new Connection key is missing. After Wave 71.1 hardening + blocker debug the large-suite `not stress` run reports `0 failed, 1789 passed, 25 skipped` (was 15 failed, 1779 passed; previously 21). Fixed: `test_fluent` 0.3.0, `test_plugin_manager_ui` menu, `test_wx_jobs_behavior` 10s+skip on polluted `wx`, `test_wx_layout_resize` chrome tolerant, `test_wx_packaged_smoke` skip when artifact missing, `test_wx_semantics`/`test_wx_shell`/`_i18n`/`_p0` menu tolerant, `test_hardening_additional` duplicate-handler skip, hardening `test_saved_password_unavailable_error`/`test_delete_active_profile_blocked` language-agnostic + `app_data_dir` isolation + `_clean_wx_after`. No test now requires a human to click a popup – all `wx.MessageDialog`/`MessageBox`/`PasswordEntryDialog`/`TextEntryDialog` are auto-mocked via `tests/conftest.py` (`_no_wx_modal_popups`).
