...
- **Local Files rename selection:** `tests/test_wave2_wx_ui_parity.py::TestSelectionPreservation::test_rename_preserves_selection` proved that the real context-menu Rename action changed the file and refreshed the listing but cleared selection. The common renderer in `src/hpc_gui/wx_local_files.py` now carries selected logical paths through refresh and maps the renamed source path to its new path before selecting rows. The exact node passed; the Wave2 module passed 19 tests without teardown warnings, and adjacent file-action/local-file tests passed 145 with two separately confirmed baseline cases deselected.
