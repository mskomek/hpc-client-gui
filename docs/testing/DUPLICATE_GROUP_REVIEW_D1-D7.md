...
## Current re-audit on governance-v3

The governance-v3 worktree is rooted directly on remediation base `87e1e709a879b52300a5eec660bfae04e700ee29`. The D1-D7 cleanup remains resolved with these current owners and exact revalidation: D1 `test_errors_replace_in_ssh`; D2 `test_sftp_download_upload_roundtrip_preserves_arbitrary_bytes`; D3 `test_cluster_servers_visible_without_selected_job_when_provider_supports`, `test_no_selection_state_shows_empty_state`, and `test_raw_server_status_action_dispatches_raw_result`; D4 `test_v4_optional_sections_ok`; D5 visible GUI owner `test_wx_local_context_menu_reopens_with_turkish_labels`; D6 catalog contract `TestOutputsBehavior::test_standard_output_localized` plus visible runtime owner `TestWave80Audit::test_runtime_language_switch_updates_outputs`; D7 service owner `ConnectionProfileServiceTests::test_typed_password_precedes_saved_secret` plus broader wx owner `test_typed_password_precedence`.

Results on the current tree: D1/D2/D4/D7-service 4 passed in 0.52s; Wave78 D3 three owners 3 passed and full file 16 passed in 12.67s; D5 visible menu 1 passed; D6 contract/runtime plus D7 wx owner 3 passed in 1.19s. The former `test_raw_server_status_opens` is not treated as an equivalent rename: its selected-job/status-label assertion is now covered by stronger, distinct status-table/raw-result and raw-viewer dispatch owners. D5's historical key-level duplicate nodes remain retired; current visible localization evidence is a separate GUI behavior owner.
