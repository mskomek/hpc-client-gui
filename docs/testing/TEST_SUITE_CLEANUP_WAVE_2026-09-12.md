...
Wave 78/D3 review checkpoint (2026-09-13): all 16 current tests passed after isolating language persistence to tmp_path. The no-selection empty-state node, provider server-table-without-selected-job node, and raw server-status viewer-action node own distinct behavior. The frozen raw_server_status_opens test selected a job and asserted only loaded status; current behavior is owned more strongly by the visible server-table/raw-result test plus the actual raw-viewer dispatch test. No direct-equivalent rename is claimed.

## Continuation reconciliation (2026-09-13; supersedes stale checkpoints)

The reconciled remediation base is `1de2dce3aa8af037033882b26029fb33f39f9f57`. Fresh clean-worktree collection reports **2,679 nodes, 0 errors**. The older `87e1e709` archive also contains 2,679 nodeids; its summary field saying 2,678 was incorrect. The prior committed remediation has 2,678 nodeids. Removing the two resolved WebView xfail annotations did not change collection.

The v4 governance branch is based on the new remediation base and has the same source tree as v3 (`fe1943dbf18c3fb46f8510f1c5667cb5204361ba`). Current collection is **2,649**, with **0 errors**. Semantic review now covers **2,649/2,649** nodes from actual test bodies and pytest markers. `python scripts/check_test_taxonomy.py --mode enforce` passes with zero-primary 0, multi-primary 0, unknown markers 0, and semantic-review gaps 0.

The confirmed Outputs language-switch defect is fixed in the existing localization architecture and its visible behavior owner passes. The release runner still finds a separate confirmed permission-denial product defect in `src/hpc_gui/wx_local_files.py`; a renamed-row selection defect and an asynchronous callback-after-destroy defect also remain confirmed. They were not changed. The separator lifecycle case remains an unresolved native wx timeout/heap-corruption failure. Current authoritative release and coverage results are recorded in `audit/archive/794226e/test-suite-final/release-suite-outcomes.json`; neither runner is a PASS.
