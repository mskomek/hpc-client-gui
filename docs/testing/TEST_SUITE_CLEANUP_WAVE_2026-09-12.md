...
Follow-up from new local branch `test-suite-governance-final-local-20260913` at `108cf4e8`: the official runner ran 17m 26s and passed compile/i18n/smoke, then the broad pytest child stopped near 90% with exit `4294967295` and no summary. Collection order places the stop in the wx Jobs cluster, but the exact node is unknown. The runner completed all isolated partitions afterward; normal suite verdict is **FAIL**, coverage is withheld, and Wave integration readiness remains **NOT READY TO MERGE** pending root cause and clean full validation.
